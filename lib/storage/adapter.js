...
/**
 * Calls the implemented {@link StorageAdapter#_clear}
 * @param {String} key - Shard hash to clear the data for
 * @param {Function} callback - Called with error or {@link StorageItem}
 */
StorageAdapter.prototype.clear = function(key, callback) {
  var self = this;
  ...
  this.peek(key, function(err, item) {
    ...
    return self._clear(key, function(err) {
      if (err) {
        return callback(err);
      }

      self.emit('clear', item);
      callback(null);
    });
  });
};
...
/**
 * Clear the shard data at the given key
 * @abstract
 * @param {String} key - Shard hash
 * @param {Function} callback - Called on complete
 */
StorageAdapter.prototype._clear = function(/* key, callback */) {
  throw new Error('Method not implemented');
};
