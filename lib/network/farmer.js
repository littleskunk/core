...
FarmerInterface.Negotiator = function(contract, callback) {
  ...
  if (this._maxShardSize && this._maxShardSize < dataSize) {
...
FarmerInterface.prototype._connectBridge = function(bridge, callback) {
  ...
  this.bridgeRequest(bridge.url, 'GET', path, headers, body, (err, contact) => {
    if (err && err.statusCode !== 404) {
      this._logger.info('Unable to get contact information from bridge');
      return callback(err);
    } else if (err && err.statusCode === 404) {
      this._logger.info('NodeID not registered on bridge');
    ...
    if (contact.address !== this.contact.address ||
      ...
      this._logger.info('Update contact information on bridge');
      this._updateBridgeContact(bridge, callback);
    } else {
      this._logger.info('Contact information unchanged, no update needed');
      callback();
    }
  });
}
