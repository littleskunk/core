...
FarmerInterface.Negotiator = function(contract, callback) {
  ...
  if (self._renterWhitelist) {
    var allowed =  {
      renterNodeId: self._renterWhitelist.indexOf(
        contract.get('renter_id')
      ) !== -1,
      renterExtendedPubKey: self._renterWhitelist.indexOf(
  ...
  self.storageManager.load(contract.get('data_hash'), function(err, item) {
    if (err) {
      self._logger.debug('no storage item available for this shard: %s', err.message);
      return callback(true);
    }
...
FarmerInterface.prototype.join = function(callback) {
  ...
  Network.prototype.join.call(self, function(err) {
    if (err) {
      return callback(err);
    }
...
FarmerInterface.prototype.getPaymentAddress = function() {
  if (this._options.paymentAddressList) {
    return this._options.paymentAddressList[Math.floor(Math.random()*(this._options.paymentAddressList.length - 1))];
  } else {
...
FarmerInterface.prototype._negotiateContract = function(contract, contact) {
  ...
  var paymentAddress = self.getPaymentAddress();
  self._logger.debug('Using Payout Address: %s', paymentAddress);
  contract.set('payment_destination', paymentAddress);
...
FarmerInterface.prototype._removeContractFromPendingList = function(contract) {
  var id = contract.get('data_hash') + contract.get('renter_id');
  var index = this._pendingOffers.indexOf(id);
...
FarmerInterface.prototype._handleContractPublication = function(contract) {
  ...
  self._logger.debug('received contract offer...');
  ...
  self._shouldSendOffer(contractObj, function(shouldSendOffer) {
    if (!shouldSendOffer) {
