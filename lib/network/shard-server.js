  ...
  this._manager.load(hash, function(err, item) {
    ...
    req.on('end', function() {
      ...
      self._manager.save(item, function(err) {
        self._log.debug('Upload marked as downloadable');
      });
  ...
  this._manager.load(hash, function(err, item) {
    ...
    function _handleTransferFinish() {
      ...
      self._manager.load(item.hash, function(err, downloaditem) {
        ...
        downloaditem.downloadimpossible = null;
        if (item.getContract(contact).get('store_begin') + 1800000 < Date.now()) {
          downloaditem.downloadcount++;
        } else {
          downloaditem.mirrorcount++;
        }
        self._manager.save(downloaditem, function(err) {
          self._log.debug('Downloadcount %s Mirrorcount %s', downloaditem.downloadcount, downloaditem.mirrorcount);
        });
      });
