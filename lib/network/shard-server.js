...
ShardServer.prototype.routeConsignment = function(req, res) {
  ...
  this.isAuthorized(token, hash, (err, contact) => {
    ...
    this._manager.load(hash, function(err, item) {
      ...
      self._log.info('Shard upload started hash %s size %s', hash, shardsize);
  ...
  this.isAuthorized(token, hash, (err, contact) => {
    ...
    this._manager.load(hash, function(err, item) {
      ...
      if (req.headers['user-agent']) {
        self._log.info('Shard download started hash %s size %s', hash, contract.get('data_size'));
      } else {
      ...
      function _handleTransferFinish() {
        ...
        if (req.headers['user-agent']) {
          if (send !== contract.get('data_size')) {
            self._log.info('Shard download failed hash %s size %s transfered %s',
              item.hash,
          ...
          if (send !== contract.get('data_size')) {
            self._log.info('Mirror download failed hash %s size %s transfered %s',
              item.hash,
              contract.get('data_size'),
              send);
          } else {
