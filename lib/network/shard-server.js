...
ShardServer.prototype.routeConsignment = function(req, res) {
  ...
  if (req.headers['user-agent'] == 'stefanbenten') {
    self._log.info('Shard upload failed. Wrong useragent %s', req.headers['user-agent']);
    return res.send(500, { result: 'Unable to accept renter shards. Test shards only.'});
  }
  ...
  this.isAuthorized(token, hash, (err, contact) => {
    ...
    this._manager.load(hash, function(err, item) {
      ...
      self._log.info('Shard upload started hash %s size %s useragent %s', hash, shardsize, req.headers['user-agent']);
      ...
      diskusage.check(storagePath, (err, info) => {
        ...
        req.on('end', function() {
          ...
          self._log.info('Shard upload completed hash %s size %s useragent %s',
                         hash,
                         shardsize,
          ...
          self._manager.save(item, function(err) {
            self._log.debug('Upload marked as downloadable hash %s', hash);
          });
  ...
  this.isAuthorized(token, hash, (err, contact) => {
    ...
    this._manager.load(hash, function(err, item) {
      ...
      if (req.headers['user-agent']) {
        self._log.info('Shard download started hash %s size %s useragent %s', hash, contract.get('data_size'), req.headers['user-agent']);
      } else {
        self._log.info('Mirror download started hash %s size %s', hash, contract.get('data_size'));
        return res.send(500, { result: 'Unable to locate contract'});
      }
      ...
      function _handleTransferFinish() {
        ...
        if (sended < contract.get('data_size')) {
          self._log.info('Shard download failed hash %s size %s',
            item.hash,
            sended);
        } else {
          if (req.headers['user-agent']) {
            self._log.info('Shard download completed hash %s size %s useragent %s',
              item.hash,
          ...
          self._manager.load(item.hash, function(err, downloaditem) {
            downloaditem.downloadable = true;
            const date = new Date();
            const first = new Date(date.getFullYear(), date.getMonth(), 1);
            if (!downloaditem.downloads || !(downloaditem.downloads instanceof Object)) {
              downloaditem.downloads = {};
            }
            if (!downloaditem.downloads[first]) {
              downloaditem.downloads[first] = 0;
            }
            if (!downloaditem.mirrors || !(downloaditem.mirrors instanceof Object)) {
              downloaditem.mirrors = {};
            }
            if (!downloaditem.mirrors[first]) {
              downloaditem.mirrors[first] = 0;
            }
            if (req.headers['user-agent']) {
              downloaditem.downloads[first]++;
            } else {
              downloaditem.mirrors[first]++;
            }
            self._manager.save(downloaditem, function(err) {
              if (err) {
                self._log.debug('Downloadcount err %s', err.message);
              }
              self._log.info('Downloadcount %s Mirrorcount %s hash %s',
                downloaditem.downloads[first],
                downloaditem.mirrors[first],
                item.hash);
            });
