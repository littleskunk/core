...
ShardServer.prototype.routeConsignment = function(req, res) {
  ...
  this._manager.load(hash, function(err, item) {
    ...
    req.on('end', function() {
      ...
      self._log.warn('Shard upload completed hash %s size %s',
        hash,
        shardsize);
      item.shard.end();
...
ShardServer.prototype.routeRetrieval = function(req, res) {
  ...
  this._manager.load(hash, function(err, item) {
    ...
    function _handleTransferFinish() {
      self.activeTransfers--;
      self._log.warn('Shard download completed hash %s', item.hash);
      ...
      self._manager.load(item.hash, function(err, downloaditem) {
        ...
        self._manager.save(downloaditem, function(err) {
          ...
          self._log.warn('Downloadcount %s Mirrorcount %s hash %s IP %s',
            downloaditem.downloadsi[first],
            downloaditem.mirrors[first],
            item.hash,
