...
ShardServer.prototype._handleEarlySocketClose = function() {
  ...
  this._logger.debug('activeTransfer--');
...
ShardServer.prototype.routeConsignment = function(req, res) {
  ...
  if (!isAuthed) {
    self._logger.debug('routeConsignment 401 unauthorized');
  ...
  this._manager.load(hash, function(err, item) {
    if (err) {
      self._logger.debug('routeConsignment 404 Shard not found');
    ...
    if (typeof item.shard.write !== 'function') {
      ...
      self._logger.debug('routeConsignment 304 Shard already exists');
      return res.send(304, { result: 'Consignment completed' });
    }

    let received = 0;

    self._logger.debug('activeTransfer++');
    ...
    req.on('data', function(chunk) {
      ...
      if (received > shardsize) {
        ...
        self._logger.debug('activeTransfer--');
        self.activeTransfers--;
        self._logger.debug('routeConsignment 400 FAILED_INTEGRITY shardsize');
        return res.send(400, {
    ...
    req.on('end', function() {
      ...
      if (calculatedHash !== hash) {
        ...
        item.shard.destroy(utils.warnOnError(self._log));
        self._logger.debug('routeConsignment 400 FAILED_INTEGRITY hash');
        return res.send(400, {
...
ShardServer.prototype.routeRetrieval = function(req, res) {
  ...
  if (!isAuthed) {
    self._logger.debug('routeConsignment 401 routeRetrieval unauthorized');
    return res.send(401, { result: authError.message });
  }
  ...
  this._manager.load(hash, function(err, item) {
    ...
    function _handleTransferFinish() {
      self._logger.debug('activeTransfer--');
      self.activeTransfers--;
      self._log.debug('Shard download completed');
