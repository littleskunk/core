...
Protocol.prototype.handleAudit = function(params, callback) {
  ...
  if (!Array.isArray(params.audits)) {
    self._logger.info('handleAudit Invalid audit list supplied');
  ...
  async.mapLimit(params.audits, limit, function(audit, done) {
    ...
    if (err) {
      self._logger.info('handleAudit err');
      return callback(err);
    }
...
Protocol.prototype._proveShardExistence = function(hash, chall, nid, cb) {
  this._logger.info('proveShardExistence handling single audit proof hash %s', hash);

  if (!hash || !chall) {
    this._logger.info('proveShardExistence Invalid data hash or challenge provided');
    return cb(new Error('Invalid data hash or challenge provided'));
  }

  this._network.storageManager.load(hash, function(err, item) {
    if (err) {
      this._logger.info('proveShardExistence err');
      return cb(err);
    }

    if (item.shard instanceof stream.Writable) {
      this._logger.info('proveShardExistence Shard not found');
...
Protocol.prototype.handleConsign = function(params, callback) {
  ...
  self._network.storageManager.load(params.data_hash, function(err, item) {
    if (err) {
      self._logger.info('handleConsign err load');
      return callback(err);
    }
    ...
    if (!contract) {
      self._logger.info('handleConsign Consignment is not authorized');
    ...
    try {
      ...
      self._logger.info('handleConsign err Consignment violates contract store time');
      return callback(err);
    }

    self._network.storageManager.save(item, function(err) {
      if (err) {
        self._logger.info('handleConsign err save');
        return callback(err);
      }
...
Protocol.prototype.handleMirror = function(params, callback) {
  ...
  self._network.storageManager.load(hash, function(err, item) {
    if (err) {
      self._logger.info('handleMirror err');
      return callback(err);
    }

    // NB: Don't mirror data we are not contracted for
    if (!item.getContract(params.contact)) {
      self._logger.info('handleMirror No contract found for shard');
    ...
    if (typeof item.shard.write !== 'function') {
      self._logger.info('handleMirror already have the shard');
    ...
    function _onFinish() {
      if (item.hash !== utils.rmd160(hasher256.digest())) {
        ...
        self._logger.info('successfully mirrored shard %s', JSON.stringify(item.contract));
...
Protocol.prototype.handleRenew = function(params, callback) {
  ...
  try {
    ...
    self._logger.info('handleRenew err assert');
    return callback(err);
  }
  ...
  this._network.storageManager.load(dataHash, function(err, item) {
    if (err) {
      self._logger.info('handleRenew err load');
      return callback(err);
    }
    ...
    if (!originalContract) {
      self._logger.info('handleRenew No contract found for renter_id');
    ...
    self._logger.info('handleRenew originalContract: %s', JSON.stringify(originalContract));
    self._logger.info('handleRenew updatedContract: %s', JSON.stringify(updatedContract));
    ...
    for (var i = 0; i < changedProperties.length; i++) {
      if (Protocol.RENEW_PROPS_ALLOWED.indexOf(changedProperties[i]) === -1) {
        self._logger.info('handleRenew %s cannot be changed', changedProperties[i]);
    ...
    self._network.storageManager.save(item, function(err) {
      if (err) {
        self._logger.info('handleRenew Failed to save updated contract');
