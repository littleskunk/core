...
Protocol.prototype.handleRenew = function(params, callback) {
  ...
  this._network.storageManager.load(dataHash, function(err, item) {
    ...
    self._logger.info('handleRenew originalContract: %s', JSON.strigify(originalContract));
    self._logger.info('handleRenew updatedContract: %s', JSON.strigify(updatedContract));
