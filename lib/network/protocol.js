...
Protocol.prototype.handleMirror = function(params, callback) {
  ...
  self._logger.info(
    'handling mirror request from %s', params.contact.nodeID
  );
...
Protocol.prototype.handleRetrieve = function(params, callback) {
  ...
  self._logger.info(
    'handling storage retrieve request from %s', params.contact.nodeID
  );
  
  if (!kad.utils.isValidKey(hash)) {
    self._logger.info('handleRetrieve Invalid data hash provided: ' + hash);
    return callback(new Error('Invalid data hash provided: ' + hash));
  }

  self._network.storageManager.load(hash, function(err, item) {
    if (err) {
      self._logger.info('handleRetrieve err');
      return callback(err);
    }

    if (!item.getContract(params.contact)) {
      self._logger.info('handleRetrieve Retrieval is not authorized');
      return callback(new Error('Retrieval is not authorized'));
    }

    if (typeof item.shard.write === 'function') {
      self._logger.info('handleRetrieve Shard data not found');
