...
Protocol.prototype.handleRetrieve = function(params, callback) {
  ...
  if (process.env.STORJ_ERRORMESSAGE) {
    return callback(new Error(process.env.STORJ_ERRORMESSAGE));
  }
  ...
  self._logger.info(
    'handling storage retrieve request %s',
...
Protocol.prototype.handleRenew = function(params, callback) {
  ...
  self._logger.info('handling contract renew request from');
  ...
  try {
    ...
    self._logger.error('renew catch error: %s', err);
    return callback(err);
  }

  this._network.storageManager.load(dataHash, function(err, item) {
    if (err) {
      self._logger.error('renew load error: %s', err);
      return callback(new Error('Contract not found or not authorized'));
    }
    ...
    if (!originalContract) {
      self._logger.error('renew error: No contract found for renter_id');
      return callback(new Error('Contract not found or not authorized'));
    }
    ...
    for (var i = 0; i < changedProperties.length; i++) {
      if (Protocol.RENEW_PROPS_ALLOWED.indexOf(changedProperties[i]) === -1) {
        self._logger.error('Renew contract failed: %s cannot be changed',
          changedProperties[i]);
        self._logger.error('renew error: %s cannot be changed', changedProperties[i]);
    ...
    self._network.storageManager.save(item, function(err) {
      if (err) {
        self._logger.error('renew save error: %s', err);
