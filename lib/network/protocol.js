...
Protocol.prototype.handleAudit = function(params, callback) {
  ...
  async.mapLimit(params.audits, limit, function(audit, done) {
    ...
    if (err) {
      self._logger.debug('handleAudit err');
      return callback(err);
    }
...
Protocol.prototype._proveShardExistence = function(hash, chall, nid, cb) {
  ...
  this._network.storageManager.load(hash, function(err, item) {
    if (err) {
      this._logger.debug('proveShardExistence err');
      return cb(err);
    }
...
Protocol.prototype.handleConsign = function(params, callback) {
  ...
  self._network.storageManager.load(params.data_hash, function(err, item) {
    if (err) {
      self._logger.debug('handleConsign err load');
      return callback(err);
    }
    ...
    item.downloadable = null;
    item.downloadimpossible = null;
    ...
    try {
      ...
      self._logger.debug('handleConsign err Consignment violates contract store time');
      return callback(err);
    }

    self._network.storageManager.save(item, function(err) {
      if (err) {
        self._logger.debug('handleConsign err save');
        return callback(err);
      }
...
Protocol.prototype.handleMirror = function(params, callback) {
  ...
  self._network.storageManager.load(hash, function(err, item) {
    if (err) {
      self._logger.debug('handleMirror err');
      return callback(err);
    }
...
Protocol.prototype.handleRenew = function(params, callback) {
  ...
  try {
    ...
    self._logger.debug('handleRenew err assert');
    return callback(err);
  }
  ...
  this._network.storageManager.load(dataHash, function(err, item) {
    if (err) {
      self._logger.debug('handleRenew err load');
      return callback(err);
    }
    ...
    self._network.storageManager.save(item, function(err) {
      if (err) {
        self._logger.debug('handleRenew Failed to save updated contract');
